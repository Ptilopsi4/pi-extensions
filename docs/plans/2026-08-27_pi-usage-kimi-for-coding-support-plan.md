...
- [ ] Run `npm --workspace @narumitw/pi-usage run build` and `npm exec vitest run -- packages/pi-usage/test/kimi-coding.test.ts packages/pi-usage/test/usage.test.ts packages/pi-usage/test/core.test.ts packages/pi-usage/test/generated-entry.test.ts` for the focused files, then `npm run check` and plain `npm test` for the full suite; verify the focused runner selects only the named files and all deterministic gates pass within the repository's 5,000 ms per-test limit.
