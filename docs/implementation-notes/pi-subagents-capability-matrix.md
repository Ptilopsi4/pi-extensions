...
| --- | --- | --- |
| Blocking orchestration | `subagent` implements single, parallel, chain, fan-in, panel, and explicit dependency workflows; the complete request is preflighted before launch | `src/execution.ts`, `src/panel-execution.ts`, blocking execution and workflow tests |
...
| Follow-up and retained lifecycle | `subagent_send` starts follow-up work; `subagent_manage` supports only interrupt and close; `subagent_mailbox` owns queue-only send and acknowledged read | `src/stateful-tool-params.ts`, registry and lifecycle tests |
...
| Workflow-dependent tool surface | `async-only` is the default and registers detached lifecycle plus inspection; `all` explicitly registers all seven tools; `blocking-only` registers blocking delegation, consultation, and inspection; `disabled` registers inspection only | `src/subagents-extension.ts`, settings UI and registration tests |
...
| Detached completion delivery | `next-turn` is the default non-waking delivery; opt-in `auto-resume` batches settled completions and requests at most one synthesis turn when the root is idle and has no pending input | `src/completion-delivery.ts`, completion-delivery tests |
