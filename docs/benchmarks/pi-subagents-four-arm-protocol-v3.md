...
It extends the v2 four-arm protocol without changing that historical protocol, its analyses, or its results.
...
| `v3-job` | `pi-subagents-v3` | Use `subagent-v3-start` once per child and `subagent-v3-wait` once per child. |
...
Parent-visible cost is not comparable across arms because call counts differ and v3 does not propagate child subprocess usage into parent session statistics.
